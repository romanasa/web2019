...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        if (method == null || beforeMethod == null || afterMethod == null) {
            throw new NotFoundException();
        }
        ...
        try {
            ...
            if (parameters.length == 0) {
                ...
                method.invoke(page, (parameters[0] == Map.class) ? view : request);
            } else {
                if (parameters[0] == Map.class) {
    ...
    private Method getMethod(Class<?> pageClass, String name) {
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                List<Method> methods = Arrays.stream(clazz.getDeclaredMethods()).filter(gotMethod -> gotMethod.getName().equals(name)).collect(Collectors.toList());
                if (methods.size() > 0) {
